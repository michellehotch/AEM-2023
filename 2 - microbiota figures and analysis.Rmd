...
qpcr_metadata$treatment <- factor(qpcr_metadata$treatment, levels = c("Control", "DMSO", "1 ng/µL", "10 ng/µL", "100 ng/µL", "800 ng/µL"))
...
genecountotal_data_long$treatment <- factor(genecountotal_data_long$treatment, levels = c("Control", "DMSO", "1 ng/µL", "10 ng/µL", "100 ng/µL", "800 ng/µL"))
...
braycurtdm_fvg1<-read_qza("results/bray_curtis_distance_matrix_fvg.qza")

braycurtdm_fvg2<-read_qza("results/fvg2_bray_curtis_distance_matrix.qza")
...
```{r}
braycurt_fecal<-read_qza("results/fecal_bray_curtis_pcoa_results.qza")

braycurt_fvg1<-read_qza("results/fvg_bray_curtis_pcoa_results.qza")

braycurt_fvg2<-read_qza("results/fvg2_bray_curtis_pcoa_results.qza")
```

##Shannon Diversity

```{r}
shannon_fecal<-read_qza("results/fecal_shannon_vector.qza")

shannon_fvg1<-read_qza("results/fvg_shannon_vector.qza")

shannon_fvg2<-read_qza("results/fvg2_shannon_vector.qza")
```
...
Fecal stacked relative, fecal stacked 16S, total 16S
...
genecount_lother %>% 
  ...
  filter(!(day =="Gut")) %>% 
  group_by(samp_id, treatment, day, sc_id) %>% 
...
ggarrange(figgc_total + rremove("ylab"),
        ggarrange(
            ...
            figgc_entero+ rremove("ylab") + rremove("xlab"), 
...
need figure for the t-test results, probably into supplemental, maybe?
...
figcorrel_bifido<-
  ...
  #geom_abline(lty = 2, color = "#f08944")+
...
Total_model2 <- lmer(total ~ treatment + day + age + (1|sc_id/mc_id), data = Total_modeldata)

anova(Total_model2)

check_model(Total_model2)

Total_model2_means = emmeans(Total_model2, specs = pairwise ~ day)
Total_model2_means
```
...
Entero_model2 <- lmer(total ~ treatment + day + age + (1|sc_id/mc_id), data = Entero_modeldata)

anova(Entero_model2)


check_model(Entero_model2)

Entero_model2_means = emmeans(Entero_model2, specs = pairwise ~ day)
Entero_model2_means
```
...
```{r}
physeq_fvg <- qza_to_phyloseq(features = "results/11_dada2_table_fvg.qza",
                              taxonomy = "results/7_taxclass_BEE_ASVs_fvg.qza",
...
```{r}
physeq_fvg2 <- qza_to_phyloseq(features = "results/11_dada2_table_fvg2.qza",
                              taxonomy = "results/7_taxclass_BEE_ASVs_fvg2.qza",
